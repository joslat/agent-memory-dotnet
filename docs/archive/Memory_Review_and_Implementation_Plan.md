...
>
> **Update 2026-06-12 — verification pass + new strategic direction (decay / bitemporal).** A 4-verifier adversarial code-check re-confirmed two things against the live tree: (1) **Part 0's isolation fixes genuinely landed** (every sampled `leak-1…7` / `corr-1/2/3` / `decay-2/3/4` is present in code, no overstatement), and (2) **three new design docs are accurate** — they surface the next major workstream, now captured in the new **§II.8 (Decay redesign & bitemporal substrate)**. In brief: decay currently does irreversible `DETACH DELETE`, the ACT-R retention score and the `GraphRetriever` hop-distance are **computed but never used for ranking**, and `invalidated_at` is **read but never written** (orphan). Companion docs: [`decay-improvement-proposal.md`](decay-improvement-proposal.md), [`bitemporal-memory-assessment.md`](bitemporal-memory-assessment.md), [`upstream-issue-memory-decay-bitemporal.md`](upstream-issue-memory-decay-bitemporal.md); upstream v0.5.0 reference snapshot in `docs/reference/neo4j-agent-memory/python-v0.5.0/`. **Status of record (2026-06-12):** branch `remediation/analysis-review-hardening` = **69 commits ahead of `main`, 70 ahead of `origin/main`** (release `v0.1.0-preview.1` tagged **locally, NOT pushed**; PR not opened — on hold per maintainer). The various inline unit/integration test counts are **date-stamped per-section snapshots, not contradictions** (the suite grows each session — re-run it to record a single current figure before tagging). Part III below is the **pre-fix snapshot**; its findings are closed in Part 0 (see the reconciliation banner there).
...
**[rel-3] ➖ USER ACTION (not auto-executed) — release ships from an un-merged branch.**
- *State (verified):* `main..HEAD = 63` commits, `origin/main..HEAD = 64`, tag `v0.1.0-preview.1` local-only (not pushed), no PR, local HEAD diverged from the pushed `origin/remediation/...` tip.
- *Disposition:* opening the PR and pushing the tag are **outward-facing/irreversible** and reserved for the maintainer. Documented here and in §III; **not executed by the agent.** Recommended: open PR → review → merge → move/push tag on the merge commit.
...
>
> **⚠️ NOTE (2026-06-12 — interim, not final):** wiring this adapter activated the **destructive `DETACH DELETE` prune**, which the subsequent decay/bitemporal reassessment identifies as the single most dangerous behavior in the repo. The prune is **correct and owner-scoped, but irreversible by design** — treat it as *interim*. The decay direction is now superseded by **§II.8**: the prune should become **non-destructive** (soft-invalidate/archive, with an explicit opt-in hard-purge for GDPR/TTL), and the retention score it computes should feed a **re-ranker** rather than a delete gate. See `docs/archive/decay-improvement-proposal.md` + `docs/archive/bitemporal-memory-assessment.md`.
...
### II.8 Decay redesign & bitemporal substrate — relevance + safety workstream (scoped 2026-06-12)

> **New direction — designed, code-verified, NOT yet implemented.** This supersedes the "decay = a wiring/owner-scoping problem" framing in §II.1/§II.6/§II.6b. Companion design docs (in `docs/`): [`decay-improvement-proposal.md`](decay-improvement-proposal.md) (structural-first decay / composite re-ranker), [`bitemporal-memory-assessment.md`](bitemporal-memory-assessment.md) (invalidate-not-delete substrate), [`upstream-issue-memory-decay-bitemporal.md`](upstream-issue-memory-decay-bitemporal.md) (alignment with upstream issue #42). Upstream v0.5.0 reference: `docs/reference/neo4j-agent-memory/python-v0.5.0/`.

**Why this is the next major workstream.** A 2026-06-12 adversarial code-verification pass (4 parallel verifiers) **confirmed five load-bearing facts against the live code**:
1. **Decay deletes irreversibly.** `DecayQueries.BuildPrune` ends in `DETACH DELETE` for Entity/Fact/Preference below the retention threshold, executed live by `Neo4jMemoryDecayService.PruneExpiredMemoriesAsync` + `agentmemory decay`. No soft path. This is the **single most dangerous behavior** in the repo (irreversible loss, breaks audit/reproducibility, a memory-poisoning attack surface) and it is the **outlier** vs every leader (Zep/Graphiti/Mem0 never hard-delete) and vs upstream (non-destructive `:SAME_AS` / `archived=true` / `supersede_preference`).
2. **The ACT-R retention score is computed but never ranks.** `confidence·e^(−λ·daysSinceAccess) + boost·accessCount` exists only in `ComputeScore` / `CalculateRetentionScoreAsync` and the prune `WHERE`; every `SearchByVector` (Fact/Entity/Preference) orders by **raw cosine `score DESC` only**. Right signal, wired to a delete gate instead of to ranking. (Recall-time reinforcement — `last_accessed_at`/`access_count` bump — *is* already wired in `MemoryService.RecallAsync`, so the inputs are live.)
3. **`GraphRetriever` computes hop-distance then discards it.** `BuildTraversalCypher` does `length(path) AS hops`, orders by `score DESC, hops ASC`, surfaces `hops` only as metadata — **no `γ^hops` per-hop decay**, and only `RELATED_TO` is traversed multi-hop (`ABOUT`/`MENTIONS`/`SAME_AS`/`TOUCHED` are written but never walked).
4. **`invalidated_at` is an orphan.** Read by all six `TemporalQueries` as-of filters but **written by nothing** — no `SchemaConstants` constant, no `SET`, no `InvalidateAsync` — so `invalidated_at IS NULL` is always true and the as-of transaction-time path is effectively dead.
5. **A second destructive path** — `ConsolidationQueries.RemoveDuplicatePreferences` `DETACH DELETE`s older duplicates (vs upstream's `:SAME_AS` keep-both). `IConflictDetectionService` is detect-only (never closes the loser).

**Phased plan** (value-for-risk order; `[additive]` = safe / `[behaviour change]` = needs an opt-in flag):

| Phase | Scope | Effort | Risk | Tag |
|---|---|---|---|---|
| **D1** ✅ | **Recency re-ranker** — blend the already-computed retention score into retrieval ranking (`R = w_sem·S_sem + w_tmp·S_tmp`). Inputs are live (reinforcement already wired); no schema change. **DONE 2026-06-12** (opt-in via `MemoryRankingOptions.RecencyWeight` / `MemoryProfile`; unit + live-Neo4j reorder tests green). | M | low | `[additive]` |
| **D2** ✅ | **Structural / hop decay** — multiply the *discarded* `GraphRetriever` hops into the score (`seedScore·γ^hops`); age-neutral, graph-native (spreading activation / PPR-lite). Rewiring of existing traversal, no plugin. **DONE 2026-06-12** (opt-in via `StructuralDecayGamma` / `MemoryProfile`; unit + live-Neo4j γ-decay test green). | M | low | `[additive]` |
| **D3** ✅ | **Query-intent presets** — `RecallOptions.Intent` (`Default`/`Latest`/`Analog`) re-weights a recall over the configured ranking; threaded via the ambient `IMemoryRankingContext` (no interface change). **DONE 2026-06-12** (unit + live-Neo4j: Latest promotes fresh, Analog keeps the precedent). *Composite `S_sem+S_str+S_tmp` unification / RRF deferred — the linear blend + presets cover the use cases.* | M | low–med | `[additive]` |
| **D4** ✅ | **Non-destructive decay** — `DecayQueries.BuildPrune` `DETACH DELETE` → `SET invalidated_at`; explicit hard-purge retained for GDPR/TTL. **DONE 2026-06-12** — **non-destructive is the default** (`MemoryDecayOptions.NonDestructive=true`; idempotent; owner-scoped; opt-in hard-delete). Live-Neo4j tests green. | M | med | `[behaviour change → safe default]` |
| **D5** ✅ | **Transaction-clock writer** — `SchemaConstants.Properties.InvalidatedAt` + owner-scoped `InvalidateAsync` on the 3 long-term repos; live recall now excludes invalidated nodes (no-op today), as-of recall keeps them pre-invalidation. **DONE 2026-06-12.** `SupersedeAsync`/service-level exposure deferred to D7. | M | low | `[additive]` |
| **D6** ✅ | **Two-timestamp recall** — `RecallAsOfAsync(validAsOf, systemAsOf)` + `AssembleContextAsOfAsync(validAsOf, systemAsOf)` + `TemporalQueries` variants; the existing single-`asOf` delegates with both equal. **DONE 2026-06-12.** Valid-time clock bounds a fact's `valid_from`/`valid_until`; transaction-time clock (`systemAsOf`) bounds every record's `created_at`/`invalidated_at` — facts observe both, messages/entities/preferences/traces observe only `systemAsOf`. Threaded through `IMemoryRecall`/`MemoryService`/`InstrumentedMemoryService` + the assembler. Unit tests (clock propagation + assembler mapping) + live-Neo4j two-clock test (all four timestamps pinned; each clock proven to filter independently) green. | M | low–med | `[additive]` |
| **D7** ✅ | **Contradiction → supersession** — repo `SupersedeAsync` writers (Fact + Preference) close a loser non-destructively (`invalidated_at` + fact `valid_until`) and link `(loser)-[:SUPERSEDED_BY]->(winner)`; opt-in `IConflictDetectionService.ResolveFactContradictionsAsync` keeps the highest-confidence assertion and supersedes the rest (R1-scoped, detection stays the default); `RemoveDuplicatePreferences` is now non-destructive + idempotent (soft-invalidate + `:SUPERSEDED_BY`, no `DETACH DELETE`). **DONE 2026-06-13.** New `SchemaConstants.RelationshipTypes.SupersededBy`. Unit (Supersede/dedup Cypher, owner scoping) + live-Neo4j integration (supersede drops loser from live recall, keeps as-of-before, links winner; resolution keeps highest-confidence + owner-isolated; dedup idempotent) green. Snapshot regenerated for the 2 changed consolidation consts. | M | med | `[additive, opt-in]` |

**✅ D1, D2, D3, D4, D5, D6 landed (2026-06-12).** D1/D2 are the opt-in, schema-neutral ranking rung behind a `MemoryProfile` (`Parity`→`Enhanced`→`Bitemporal`) "start at parity, dial up" switch (default `Parity` = off). D5/D4 landed the safety win — a real `invalidated_at` transaction clock + **non-destructive decay by default** (forgetting is now reversible; hard-delete is an explicit opt-in). **D3** — per-request `RecallOptions.Intent` (`latest`/`analog`/`default`) via the ambient `IMemoryRankingContext`. **D6** — bitemporal two-clock recall (`RecallAsOfAsync(validAsOf, systemAsOf)` + assembler overload) surfacing the transaction clock through the public recall API. **D7** — contradiction→supersession: repo `SupersedeAsync` writers (the piece deferred from D5), opt-in `ResolveFactContradictionsAsync`, and non-destructive duplicate-preference collapse — closing the last destructive path so forgetting is now fully reversible. **D1–D7 are all done and verified on live Neo4j — the decay/bitemporal workstream is COMPLETE.** **Parity:** the re-ranker (D1–D3) *aligns* with upstream issue #42; the bitemporal substrate (D4–D7) is a deliberate additive **superset/divergence** (neither side is bitemporal). *Note: D1/D2 changed only query-builder **methods**, not `const` strings, so `CypherQuerySnapshot.snap` + `ExpectedQueryCount` were unaffected; the later phases that add const DDL will update them.*

**Open decisions (the maintainer's to make — see each design doc's "open questions"):** default blend weights + structural `γ`; non-destructive decay **default vs opt-in** (current tests assert deletion); overload `created_at` vs add an immutable `recorded_at`; per-type pinning so safety/identity facts never sink or expire.

---

## Part III — Review Annex (in-depth audit, 2026-06-07)

> **⚠️ RECONCILIATION (read first — added 2026-06-12).** This annex is the **2026-06-07 _pre-fix_ adversarial snapshot.** Every HIGH/MEDIUM/LOW finding below (`leak-1…7`, `gap-1/2/4`, `gap-overfetch`, `corr-1…4`, `decay-1…4`, `test-1/2`, `design-1`, `rel-meta`, `cleanup`) was **subsequently implemented and is tracked as ✅ DONE in Part 0 (§0.A–§0.E)** — and re-verified against the live code on 2026-06-12 (all sampled findings confirmed genuinely landed). The present-tense "open / top item to fix" wording below is **preserved verbatim as the historical record of what was found**; for current status see Part 0. The only items still open are deliberate **USER ACTIONs / documented-intentional**: `rel-3` (open the PR / push the tag) and `leak-6`/`decay-1`.
...
*This document supersedes the multi-user/isolation aspects of earlier plans and complements `docs/archive/Implementation_Plan_Remediation.md` (closed/archived), `docs/architecture.md`, and `docs/schema.md`. The next-direction (decay/bitemporal) design lives in `docs/decay-improvement-proposal.md`, `docs/bitemporal-memory-assessment.md`, and `docs/upstream-issue-memory-decay-bitemporal.md` (see §II.8); upstream v0.5.0 reference in `docs/reference/neo4j-agent-memory/python-v0.5.0/`.*
