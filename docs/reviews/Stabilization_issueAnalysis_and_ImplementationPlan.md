...
| 5 | `ContextFormatOptions.cs`, `MemoryRecallSecurityOptions.cs` | None of the four #92 trust/security enum knobs (`SecurityMode`, `MinimumTrustForAdmissionBypass`, `DefaultMemoryRole`, `MinimumTrustForSystemRole`) are range-checked; `IConfiguration`'s enum binder accepts any integer for a numeric enum, so e.g. `"MinimumTrustForAdmissionBypass": 99` silently binds to an undefined `MemoryTrustLevel` | Add `Enum.IsDefined` validation for each, alongside the existing `MaxChatHistoryMessages` check — landed in `AgentFramework`'s and `SemanticKernel`'s own DI registrations (the internal `MemoryContextFormatterOptions` is never independently DI-bound, so needs no separate validation) |
| 6 | `AutoMergeThreshold`/`SameAsThreshold` on `ExtractionOptions` (`src/AgentMemory.Abstractions/Options/ExtractionOptions.cs`, reached via `MemoryOptions.Extraction`) | Documented as ordered but have zero validation anywhere; setting them backwards silently inverts merge-vs-relate behavior | Validate both in `[0,1]` and `SameAsThreshold <= AutoMergeThreshold` — landed in `AgentMemory.Core`'s `MemoryOptions` validation chain (`ExtractionOptions` itself is never independently DI-bound) |
...
| # | File | Finding | Fix |
|---|---|---|---|
| 1 | `docs/architecture.md` §3.4.6 | ~~MCP surface table says "25 MCP tools, 6 resources, 3 prompts"; actual is 33/12/6~~ **Correction (found during self-review): this finding was itself wrong.** Re-verified directly (`grep -c "\[McpServerTool("`, excluding the class-level `[McpServerToolType]` markers this count double-counted) — the true counts are 25/6/3, exactly matching the original doc text. No fix applied; the doc was already correct | No change (self-review caught and reverted an incorrect "fix" before merge) |
...
- **Test-coverage caveat, disclosed here per self-review:** 3 of the 10 config-validation fixes (`LongTermMemoryOptions`/`ShortTermMemoryOptions`/`ReasoningMemoryOptions`' fields, reached through `MemoryOptions.ShortTerm`/`LongTerm`/`Reasoning`) are on `sealed record` types whose properties — and the parent `MemoryOptions` slot itself — are `{ get; init; }`. That makes them unreachable through the `Action<MemoryOptions> configure` delegate every other call site in this codebase uses, so they're covered by code review and by the .NET options binder's documented support for binding `init` properties from `IConfiguration`, but not by an executable unit test in this PR (a test asserting this would need `IConfiguration`-based registration, out of scope here). `MemoryIsolationOptions`/`ExtractionOptions` are mutable classes and ARE fully unit-tested via the configure delegate.

## Self-review findings and dispositions

3 parallel finder agents (correctness, cross-file impact, cleanup/conventions) reviewed the implementation commit before merge. No functional/security bugs were found in the fixes themselves. Findings and how each was resolved:

- **Stale comments (3, all in the same family):** `MafTypeMapper.cs`, `RecalledMessageRoleGate.cs`, and `Neo4jMicrosoftMemoryFacade.cs` each had a comment (one edited by this PR, two pre-existing) claiming `Neo4jChatMessageStore`/`Neo4jChatHistoryProvider` were "intentionally ungated" for role/admission — exactly what 2.1 just fixed. All three updated to reflect the new, correct state. Real risk if left: a future maintainer trusting the stale comment could undo this fix.
- **`docs/architecture.md`'s MCP tool-count "fix" was itself wrong** — caught by the cleanup-review agent and independently re-verified directly (`grep -c "\[McpServerTool("` excluding class-level type markers): the true counts are 25 tools/6 resources/3 prompts, exactly matching what the doc said BEFORE this PR. The original Angle-5 audit finding that prompted the "fix" had double-counted class-level `[McpServerToolType]`-style marker attributes as additional tools. Reverted to the original, correct text; see the corrected Angle 5 row above.
- **Two validation-message wording inconsistencies** (`EnrichmentCacheOptions` and `MemoryStoreOptions`, both using a `ClassName.Property` style while every sibling `.Validate()` call in the same file/method uses a short-label style) — fixed to match each file's established local convention.
- **This doc's own Angle 3 rows 5 and 6 cited files that were never touched** (`MemoryContextFormatterOptions.cs`, standalone `ExtractionOptions.cs`) — corrected above to name where the fixes actually landed (`AgentFramework`/`SemanticKernel` DI registrations; `AgentMemory.Core`'s `MemoryOptions` chain), since neither of those two option types is ever independently DI-bound.
