...
- Adding `AgentMemory.Nams` to `eng/release-packages.txt` (the release-package manifest). It builds and packs like every other package (inherited from `Directory.Build.props`), but is **not yet added to the publish manifest** — a skeleton with no working backend has nothing to publish. Confirmed via `strategy/STATUS.md`'s own documented rule: "adding a project under `src/` doesn't publish it until it's added [to `eng/release-packages.txt`]".
...
- [ ] Self-reviewed via 3 parallel finder agents (this phase touches real source code — a whole new package — so the full review applies, unlike Phase 0's docs-only diff).
