...
| 7 | **Consolidation primitives** + `ConsolidationRun` node | 0.2/0.5 | (b)+(c) | **Already ported** (`Neo4jConsolidationService`, `ConsolidationQueries`, `0004_consolidation.cypher`). Check the `ConsolidationRun` bookkeeping shape matches upstream if cross-impl reads matter. At/ahead of parity. |
