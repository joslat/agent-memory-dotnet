...
|---|---|---|---|---|
| **Recall fan-out** (`MemoryOptions.FanOut`) | Per-memory-type sub-queries retrieved separately and merged into the monolithic sections by id, keeping the higher score | All five — each leg carries a `MemoryTypeAffinity` | **No** — merged sections are re-capped at the existing `MaxX`, so budgets are never multiplied | BUILT, WIRED, **UNMEASURED**, off by default (`MemoryOptions.FanOut.Enabled`) |
