...
**Tracked isolation-hardening backlog (pre-existing, documented — NOT session regressions; require knowing ids/coords; mostly internal/admin/resolution, so not active recall leaks).** These mean the "every surface owner-aware" phrasing should be read as *recall + primary CRUD*; the following reads are not yet owner-scoped: entity resolution (`GetByType`/`ResolveEntityAsync` — entities are often legitimately shared, an open product decision), spatial search (`SearchByLocation`/`SearchInBoundingBox`), `FindSimilarByEmbedding`, `GetEntitiesFromMessage`, and the dedup-admin reads (`GetPendingDuplicates`/`GetDeduplicationStats`). Decide per-path: owner-scope, or move admin reads behind an explicit system-audit surface.
