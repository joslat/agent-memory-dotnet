...
|---|---|---|---|---|---|---|
| R1 | Multi-user memory isolation (`owner_id` + `MemoryScope`, optional shared) | core / neo4j / abstractions / adapters | 🟥 HIGH | 🟡 In progress — ✅I1 abstractions · ✅I3 scoped read path (Fact/Entity/Preference `SearchByVector` owner-OR-shared filter + over-fetch; repos/service take `MemoryScope`) · ✅I4 assembler derives scope from `RecallOptions.Scope`/`RecallRequest.UserId` · next: I5 write-path `owner_id` | L | Implement before first NuGet publish; phased plan in §R1 |
