...
|---|---|---|---|---|---|---|
| R1 | Multi-user memory isolation (`owner_id` + `MemoryScope`, optional shared) | core / neo4j / abstractions / adapters | 🟥 HIGH | 🟡 In progress — ✅I1 abstractions · ✅I3 scoped read path · ✅I4 assembler scope · ✅I5 write path (`owner_id` persisted+read-back on Fact/Entity/Preference; Fact MERGE now keyed by `owner_key`; extraction stamps owner from `ExtractionRequest.UserId`) — **read+write loop closed for facts/entities/preferences**. Next: I6 indexes/migration, I9 isolation tests; Relationship/ReasoningTrace owner-write deferred (R2) | L | Implement before first NuGet publish; phased plan in §R1 |
