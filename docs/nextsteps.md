...
### Step 1 — Package Rename (Neo4j.AgentMemory.* → AgentMemory.*) ✅ DONE 2026-04-30
...
> **✅ Both shipped.** The CLI delivered well beyond the v1 scope — `agentmemory` ships migrate, bootstrap, schema-check, consolidate, decay, conflicts, schema-parity, invalidate, supersede. The planned `schema-check` (runtime DB conformance) shipped 2026-06-14, alongside the static `schema-parity` upstream-compatibility check. GDS shipped as the opt-in `AgentMemory.Analytics` package. See the status tracker (row 6 / row 7). The planning text below is the original plan, kept for context.
