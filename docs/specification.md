...
`AgentMemory.Nams` also exists under `src/` as of the NAMS backend engineering effort. It's listed in `eng/release-packages.txt` (a mandatory, CI-checked inventory of every `src/*` package, not an opt-in publish gate). As of Phase 2 it has a configuration surface plus a low-level REST client, retry policy, and error model (its own `HttpClient`-based implementation calling the NAMS REST API directly — no dependency on the external `Neo4j.AgentMemory` TCK client), as of Phase 3 an identity/conversation-resolution subsystem (`INamsConversationStateStore`/`INamsConversationResolver`), as of Phase 4 a recall/context-mapping subsystem (`INamsRecallService`, deliberately unescaped/undelimited/ungated — this package cannot reference the types real gating requires), and as of Phase 5 a post-turn persistence subsystem (`INamsPersistenceService`, one bulk write per turn, classified `Persisted`/`Failed`/`UnknownWriteOutcome`) — see `docs/reviews/NAMS_Phase2_LowLevelClientAdapter_PlanningAndImplementationPlan.md`, `docs/reviews/NAMS_Phase3_IdentityAndConversationMapping_PlanningAndImplementationPlan.md`, `docs/reviews/NAMS_Phase4_RecallAndContextMapping_PlanningAndImplementationPlan.md`, and `docs/reviews/NAMS_Phase5_PostTurnPersistence_PlanningAndImplementationPlan.md`.

A new `AgentMemory.AgentFramework.Nams` package (Phase 6) also exists under `src/` — the dedicated MAF adapter for the NAMS backend (`NamsMemoryContextProvider`), kept separate from both `AgentMemory.Nams` (framework-free) and `AgentMemory.AgentFramework` (backend-neutral for the direct provider) per ADR-9. This is where Phases 3-5's raw, ungated content finally gets escaped, delimited, and admission-checked — see `docs/reviews/NAMS_Phase6_DedicatedMafProvider_PlanningAndImplementationPlan.md`.
