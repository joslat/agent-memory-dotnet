...
### Doc Review Sprint (2026-04-30)

Reviewed `docs/getting-started.md` and `docs/architecture.md` §3.3 for Neo4j/persistence accuracy.

**Confirmed correct:**
- DI method name: `AddNeo4jAgentMemory()` ✅
- GraphRAG adapter method name: `AddGraphRagAdapter()` ✅ (separate call, not bundled into `AddNeo4jAgentMemory()`)
- Config keys `Neo4j:Uri`, `Neo4j:Username`, `Neo4j:Password` match `Neo4jOptions` properties ✅
- `SchemaBootstrapper` registered as `ISchemaBootstrapper` (transient) ✅
- `Neo4j.AgentMemory.Neo4j.csproj` does NOT reference `Microsoft.Agents.*` ✅

**Issues found:**
1. `getting-started.md` §3.3: wrong type `Neo4jSchemaBootstrapper` — no such type exists; correct resolve is `ISchemaBootstrapper` (High)
2. `getting-started.md` §3.2: `Neo4jOptions.Database` property (default "neo4j") not documented — silent misconfiguration risk (Medium)
3. `getting-started.md`: `AddGraphRagAdapter()` entirely absent — omission not error (Low)
4. `architecture.md` §3.3: `Microsoft.Extensions.AI.Abstractions 10.4.1` missing from Dependencies list — it IS in the csproj (Low)

Review output written to `.squad/decisions/inbox/gaff-review-2026-04-30.md`.
...
This history was summarized to keep the file under 15,360 bytes. Key learnings are preserved above. Earlier explorations have been archived to .squad/history-archive/gaff.md.

**History Size Before:** 34247 bytes
