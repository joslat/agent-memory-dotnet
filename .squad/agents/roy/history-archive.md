...
### 2026-07-25: Full Parity Snapshot — Post Doc-Cleanup Sprint

**Task:** Evidence-based parity assessment of complete .NET implementation vs Python reference, covering all dimensions (not just schema/Cypher).

**Overall Verdict: MIXED — infrastructurally ahead, ecosystem-breadth behind**

**.NET AHEAD on:**
- Memory decay (MemoryDecayService — no Python equivalent)
- Point-in-time temporal queries (RecallAsOfAsync, TemporalQueries.cs — no Python equivalent)
- BM25/fulltext search (3 fulltext indexes + FulltextRetriever — no Python equivalent)
- Hybrid retrieval (vector + BM25 — no Python equivalent)
- GraphRAG multi-hop expansion (RELATED_TO*1..2 — no Python equivalent)
- Batch UNWIND operations (entity/fact upsert — no Python equivalent)
- Migration versioning (MigrationRunner — no Python equivalent)
- Multi-extractor merge strategies (5 strategies — Python has none)
- Architecture rigor (ports-and-adapters, zero boundary violations, 11 clean packages vs Python monolith)
- Schema richness (~99.7% parity + 14 extra props, 3 extra rel types, 3 fulltext indexes)
- Cypher functional parity (98.5%, 145 constants vs Python's 99)

**AT PARITY on:**
- Core domain model (Conversation, Message, Entity, Fact, Preference, Relationship, ReasoningTrace, ReasoningStep, ToolCall)
- Vector search across all memory layers
- Entity resolution chain (Exact→Fuzzy→Semantic→New)
- LLM-based extraction pipeline
- MCP server (21 tools / 6 resources / 3 prompts vs Python's _tools.py/_resources.py)
- Enrichment (Wikimedia + Nominatim geocoding)
- OpenTelemetry observability
- MAF/Microsoft Agent Framework integration

**.NET BEHIND on:**
- Framework integrations: 3 (.NET) vs 9+ (Python: crewai, google_adk, langchain, llamaindex, openai_agents, pydantic_ai, strands, agentcore, microsoft_agent)
- Local NLP extractors: 0 (.NET) vs 2 (Python: GLiNER, spaCy)
- Streaming extraction for long documents: missing in .NET (Python has chunked streaming, token overlap, async generators)
- Local embedding providers: .NET has MEAI abstraction but no concrete sentence-transformers adapter; Python ships 4 backends
- CLI tool: Python has click CLI (833 LOC); .NET has none
- GDS integration: Python's microsoft_agent/gds.py; .NET has none
- Opik observability: Python has opik.py; .NET OTel only
- Benchmarks: Python has 3; .NET has 0

**Key numbers (current):**
- .NET: 11 packages, 315 .cs files, ~17,743 LOC, **1,477 test methods**
- Python: 1 package (15 modules), 96 .py files, ~26,169 LOC, ~1,792 test functions

**Next engineering moves (priority order):**
1. NuGet release prep (highest value unlock, code is ready)
2. Streaming extraction (IStreamingExtractionPipeline)
3. Local sentence-transformers embedding adapter
4. Additional framework integrations (AutoGen, LangChain.NET)
5. DELETE_SESSION_DATA gap closure (#51)
6. BenchmarkDotNet harness

**Artifacts created:**
- `.squad/decisions/inbox/roy-parity-snapshot.md` — full evidence-based parity snapshot
...
This history was summarized to keep the file under 15,360 bytes. Key learnings are preserved above. Earlier explorations have been archived to .squad/history-archive/roy.md.

**History Size Before:** 55179 bytes
...
### 2026-04-30: API Accuracy Review — getting-started.md, CONTRIBUTING.md, README.md

**Task:** Cross-check all interface names, method signatures, type names, and namespace references in three recently created/updated docs against the actual implementation.

**Findings:**

- **CONTRIBUTING.md** — Approved. All architectural descriptions, domain conventions, stub locations, and reviewer checklist items are accurate.

- **docs/getting-started.md** — 3 issues:
  1. §3.3: GetRequiredService<Neo4jSchemaBootstrapper>() should be GetRequiredService<ISchemaBootstrapper>() — the concrete class is SchemaBootstrapper, registered as ISchemaBootstrapper.
  2. §4: 
ecall.Messages.Count / 
ecall.Entities.Count — these properties don't exist on RecallResult. The real shape is 
ecall.Context.RecentMessages.Items.Count / 
ecall.Context.RelevantEntities.Items.Count via MemoryContext → MemoryContextSection<T>.
  3. §4.1: 
ew Message { ... } initializer missing 
equired properties MessageId and TimestampUtc — won't compile.

- **README.md** — 2 issues in the Quick Start block:
  1. options.ConnectionUri / options.AuthToken don't exist on Neo4jOptions. Correct properties are Uri, Username, Password.
  2. 
ew Neo4jSchemaBootstrapper(driver) — wrong class name and can't be directly instantiated. Must use DI: provider.GetRequiredService<ISchemaBootstrapper>().

**Key insight confirmed:** RecallResult wraps everything inside Context: MemoryContext, which then exposes typed MemoryContextSection<T> sections — docs must never flatten this to top-level .Messages / .Entities.

**Action:** Full findings written to .squad/decisions/inbox/roy-review-2026-04-30.md for Joi to apply corrections.
