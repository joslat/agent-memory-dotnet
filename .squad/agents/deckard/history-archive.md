...
- **Before:** B1 stated "Abstractions MUST NOT reference any NuGet package" (absolute prohibition), while the verification block below it listed a `⚠️ B1 exception` for `Microsoft.Extensions.AI.Abstractions`. The rule and the verified reality were contradictory. Additionally, B4 and B6 referenced the phantom `Neo4j.AgentFramework.GraphRAG` package (never a dependency, referenced only as historical context); their rationales were factually wrong after the GraphRAG retrieval merge into `Neo4j.AgentMemory.Neo4j`.
- **After:** B1 now explicitly names `Microsoft.Extensions.AI.Abstractions` as the single approved exception with decision reference D-AR2-1. B4 and B6 now describe the actual constraint (no framework adapter SDKs) without referencing a non-existent package. Verification block corrected: Abstractions .csproj has ONE PackageReference (M.E.AI.Abstractions 10.4.1), not zero.

**Key priority recommendations from nextsteps.md matrix (2026-04-30):**
- **Do first:** NuGet Release Prep (V=9, C=2, HIGH) — highest leverage unlock; then `DELETE_SESSION_DATA` fix (V=5, C=1, HIGH — one Cypher change, negligible cost); then AutoGen.NET integration (V=6, C=3, HIGH).
- **Do next:** Streaming Extraction (V=7, C=5, MED) + Local Embedding Adapter (V=6, C=5, MED) — close the two most user-visible functional gaps.
- **Defer:** Local NLP extractors (no .NET GLiNER binding, HIGH uncertainty), Opik (blocked on upstream .NET SDK), GDS (operational complexity, post-v1).



Synthesised all parity findings (Roy's session snapshot), doc overhaul decisions (Joi's session), and remaining improvement-suggestions into a single durable `docs/nextsteps.md`. Key architectural conclusions:

1. **Verdict is "mixed" on a single axis, "ahead" on infrastructure depth.** .NET wins on decay, temporal recall, BM25/hybrid search, migrations, GraphRAG, architecture rigour, multi-extractor merge strategies. Python wins on framework integrations (9 vs 3), local NLP (GLiNER/spaCy), streaming extraction, local embedding backends, CLI, GDS, Opik.

2. **Release prep is the right Step 1.** No code work is blocked on it; it is purely scaffolding (CHANGELOG, CONTRIBUTING, package metadata, CI push workflow). Publishing makes feedback possible.

3. **Streaming extraction (Step 2) is the highest-value functional gap.** Python's `streaming.py` provides the design reference. Clean fit as a new interface in Abstractions + Core implementation. No breaking changes.

4. **Local embedding adapter (Step 3)** is second meaningful gap for cloud-free deployments. MEAI abstraction already correct; only a concrete ONNX/sentence-transformers implementation is missing.

5. **Framework integrations (Step 4) are the most visible ecosystem gap.** AutoGen.NET is the recommended first target. Each follows the existing AgentFramework package template.

6. **`DELETE_SESSION_DATA` partial gap (#51) is the only genuine non-decided parity gap.** Very small: one Cypher change in ConversationRepository.

7. **All items from improvement-suggestions.md are either complete, deferred (S7/S9), or not recommended.** No new architecture improvements were identified by this synthesis.

8. **Key file:** `docs/nextsteps.md` — written as the authoritative forward-looking doc after the doc cleanup sprint.
...
This history was summarized to keep the file under 15,360 bytes. Key learnings are preserved above. Earlier explorations have been archived to .squad/history-archive/deckard.md.

**History Size Before:** 68933 bytes
...
### 2026-05-08T09:26:42.925+02:00 — Squad/Model/Status Assessment

**Architecture / status learnings:**
- Current operational truth is split: `.squad/identity/now.md` is the newest status artifact and says all phases plus gap-closure are complete, with focus on NuGet release prep, streaming extraction, local embedding, and additional framework integrations.
- `docs/nextsteps.md` still carries older sequencing that treats Aspire Demo as an active started item and a release gate. This is a governance drift signal; reconcile `now.md` and `docs/nextsteps.md` before routing more work.
- The shipped codebase currently has 11 source packages under `src\` and 3 test projects under `tests\`.
- Package boundaries remain sound in project references: Abstractions is the leaf, Core depends only on Abstractions, Neo4j depends on Core + Abstractions, adapters depend inward.
- GraphRAG retrieval remains correctly internalized into `src\AgentMemory.Neo4j\`; there is no separate `GraphRagAdapter` package to revive.

**Validation / runtime learnings:**
- Unit health in this environment is strong: `AgentMemory.Tests.Unit` passed 2,058 tests and `AgentMemory.Tests.Unit.SemanticKernel` passed 31 tests.
- Integration validation is environment-gated here: `AgentMemory.Tests.Integration` failed because Docker was unavailable at `npipe://./pipe/docker_engine`, not because of a product-code compile failure.

**Model / user-preference learnings:**
- Current persistent squad model policy is centered on `claude-sonnet-4.6` for default, implementation, and deep analysis; `gpt-5.3-codex` for heavy codegen; `claude-haiku-4.5` for mechanical work.
- Runtime-usable non-excluded models for planning/spawns are: `claude-sonnet-4.6`, `claude-sonnet-4.5`, `claude-haiku-4.5`, `gpt-5.4`, `gpt-5.3-codex`, `gpt-5.2-codex`, `gpt-5.2`, `gpt-5.4-mini`, `gpt-5-mini`, and `gpt-4.1`.
- User directive for this assessment: avoid `claude-opus-4.7` and `gpt-5.5` completely.
