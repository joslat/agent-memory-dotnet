...
public sealed class AbstractionsContractGuardTests
{
    ...
    private const int DocumentedDomainRecords = 75; // +ForgottenTopicSummary (30.8 — a summary, never the forgotten facts: rendering the content would undo the forgetting, putting the decayed values back in the prompt to be answered from); +ProspectiveDueResult (30.7 — two lists rather than one, because "just became true" and "about to stop being true" are different claims and a reader who has to infer which from the dates ignores the block); +MemoryDelta, +SupersededFactPair, +SupersededPreferencePair, +MemoryDeltaRequest, +FactDeltaRows, +PreferenceDeltaRows (30.5 delta recall — the two *DeltaRows records are the repository-boundary shape, kept distinct from MemoryDelta so a repository cannot silently return a partially-assembled delta); +EntitySummary (PLAN 12.7 / S1; EntitySummarySource is a record STRUCT and the guard counts reference records only); +MemoryBlock, +MemoryBlockLine (PLAN 12.9 / S4); +BulkIngestionOptions, +BulkIngestionOutcome, +BulkIngestionResult (PLAN 2.9 / rank 27); +ProjectedContext, +ProjectedItemAnnotation, +ProjectedBlock (30.2 projection layer — MemoryProjectionOptions lives in .Options and is not counted here); +RecallSubQuery, +SubQueryYield, +RecallFanOutReport (30.10 adaptive router — the request half and the witness half; the gate, deriver, planner and merge are all internal, so this is the minimum public surface that can express a per-memory-type fan-out and report what it bought); +SupersededFact (30.2 step 6: what a fact USED to say, deliberately not a whole Fact — a predecessor is read only to render "since D; previously Y", and returning full facts would carry embeddings across a boundary with no use for them)
    private const int DocumentedEnums = 34; // +DerivedFactMode (W2 — the READ side of derived memory: Include is the pre-existing behaviour and the measured-harmful one at −16 points, Exclude removes the dilution, Only gives derived facts their own budget so a count reaches the prompt without costing a source fact its slot); +TraceKind (7.1), +ExtractionProvenanceMode (9.3), +TemporalQueryClocks (13.2), +ProjectedBlockKind (30.2), +MemoryTypeAffinity (30.10 — deliberately NOT [Flags]: a sub-query targets exactly one memory type, and a bitwise union would re-express the blended monolithic query this mechanism exists to split); +DerivationOperators (30.6 — a [Flags] enum rather than a bool per operator, so a host selects arithmetic as a set and the reachability guard can assert every flag has exactly one evaluator)
