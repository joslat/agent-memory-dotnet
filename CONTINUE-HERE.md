...
1. **API-surface audit → `1.0` candidate** — lock the public surface under SemVer; fold in preview.4's surface changes (nullable `UpdateAsync`, owner-scoped `ClearSession`/`DeleteBySession`) and the newly-exposed `MergeEntitiesAsync`; decide the two library follow-ups the Silver/Gold wiring surfaced (first-class owner-scoped/paged `ListAllTracesAsync` + `IToolCallRepository.GetStatsAsync(toolName?)` — would remove the bridge's raw-Cypher fallbacks) and the merge-relationship-transfer gap.
